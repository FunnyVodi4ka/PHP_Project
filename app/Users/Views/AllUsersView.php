...
while ($row = $stmt->fetch())
{
    ...
    if(!empty($row['deleted_at'])){
        echo "<td colspan='2'><a class='btn btn-outline-secondary' href='http://localhost/users/".$row["user_id"]."/recover'>Восстановить</a></td>";
    }
    else{
        echo "<td><a class='btn btn-outline-warning' href='http://localhost/users/".$row["user_id"]."/edit'>Редактировать</a></td>";

        echo "<td><a class='btn btn-outline-danger' href='http://localhost/users/".$row["user_id"]."/delete'>Удалить</a></td>";
    }
    echo "</tr>";
}
echo "</table>";
